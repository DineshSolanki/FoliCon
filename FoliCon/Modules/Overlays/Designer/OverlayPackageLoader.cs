    ...
    public virtual OverlayPackageLoadResult Load(string overlayJsonPath)
    {
        if (string.IsNullOrWhiteSpace(overlayJsonPath) || !File.Exists(overlayJsonPath))
        {
            return OverlayPackageLoadResult.Failure(
                string.Format(Lang.OverlayLoadFileNotFound, overlayJsonPath));
        }
        ...
        {
            return OverlayPackageLoadResult.Failure(
                string.Format(Lang.OverlayLoadFolderUnresolvable, overlayJsonPath));
        }
        ...
        catch (JsonException ex)
        {
            ...
            return OverlayPackageLoadResult.Failure(string.Format(
                Lang.OverlayLoadInvalidJson, Path.GetFileName(overlayJsonPath), ex.Message));
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            Logger.Warn(ex, "Could not read overlay.json at {Path}", overlayJsonPath);
            return OverlayPackageLoadResult.Failure(
                string.Format(Lang.OverlayLoadUnreadable, overlayJsonPath, ex.Message));
        }

        if (definition == null)
        {
            return OverlayPackageLoadResult.Failure(
                string.Format(Lang.OverlayLoadNoDefinition, Path.GetFileName(overlayJsonPath)));
        }

        if (definition.SchemaVersion > OverlayConstants.appSupportedSchemaVersion)
        {
            return OverlayPackageLoadResult.Failure(string.Format(
                Lang.OverlayLoadSchemaTooNew,
                definition.SchemaVersion, OverlayConstants.appSupportedSchemaVersion));
        }
